...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
        ...
        new Web3Modal({
            ...
            providerOptions: {
                walletconnect: {
                    ...
                    options: {
                        rpc: {
                            [Networks.AVAX]: getTestnetURI(),
                            [Networks.BSC_MAINNET]: getMainnetURI(),
                            [Networks.HARMONY]: getMainnetURI(),
                        },
                    },
                },
            },
    ...
    const connect = useCallback(async () => {
        ...
        if (chainId === Networks.BSC_MAINNET || chainId === Networks.AVAX || chainId === Networks.HARMONY) {
    ...
    const checkWrongNetwork = async (): Promise<boolean> => {
        if (providerChainID !== DEFAULT_NETWORK) {
            const shouldSwitch = window.confirm(messages.switch_to_bsc);
            if (shouldSwitch) {
                await switchNetwork();
                window.location.reload();
            }
